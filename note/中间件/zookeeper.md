...
#### ZAB协议
##### 基本介绍
Zookeeper并没有采用Paxos算法，而是用ZAB来保证数据一致性核心算法。是一个特别为ZooKeeper设计的**奔溃可恢复的原子**消息广播算法。所有事务请求必须由一个**全局唯一的服务器来协调处理**（Leader），剩余的其他服务器成为Follower服务器，Leader负责把客户端的事务请求转化成一个提议，并将该提议分发给所有Follower，之后Leader服务器等待Follower反馈，超过半数正确反馈后，Leader再次向所有的Follower分发Commit。
##### 重点说明
###### 消息广播
类似于一个二阶段提交的过程。
...
###### 崩溃恢复
一旦Leader奔溃或者网络原因导致Leader服务器失去了过半Follower的联系。此时需要选举新的一个Leader服务器，同时集群中已经有过半机器与该Leader服务器完成状态同步之后，达成一致的状态，ZAB协议就退出恢复模式，进入消息广播模式，选举算法要做到集群中所有人都知道其成为Leader。
1. 基本特性
    确保已经在Leader服务器上提交的事务最终被所有服务器都提交。
    确保丢弃那些只在Leader服务器上提出的事务。
    基于上面特性，算法要保证选举出来的Leader服务器拥有集群中所有机器最高编号的事务，这样就可以保证新选举出来的Leader一定具有所有已经提交提案。
2. 数据同步
   完成Leader选举之后，正式工作之前，Leader服务器会首先确认事务日志中的Proposal是否都已经被集群中过半机器提交了，即是否完成数据同步。
   Leader为每个Follower服务器都准备一个队列，将没有被Follower同步的事务进行同步并commit，等Follower都同步完后，才会加入真正的可用Follower列表。
3. ZXID
   是一个64位的数字，其中低32可用看作市一个简单的单调递增计数器，每次的客户请求，都会对该计数器+1；高32位代表Leader周期epoch的编号，每当选举产生一个新的Leader就会从这Leader去除本地最大的ZXID，并从该ZXID中解析epoch值，对其进行+1，后续就以此编号开始，低32从0开始从新生成ZXID。
##### 与Paxos区别
...
### zookeeper技术内幕
#### 系统模型
从系统模型、节点特性、版本、Watcher、ACL来讲诉
##### 数据模型
###### 树
  每一个数据节点都被称为一个ZNode，通过/来进行区分
###### 事务ID
  在Zookeeper中，事务指的是改变ZooKeeper服务器状态的操作，成为事务操作或更新操作，一般包括数据节点创建和删除、数据节点内容更新和客户端会话创建与失效等操作，对于每个事务请求，都会为其分配一个全局唯一的事务ID用ZXID标识。从这些ID也能识别出全局的顺序。
##### 节点特性
###### 节点类型
- 持久节点
  该数据节点被创建后会一直存在ZooKeeper服务器上，知道有删除操作清楚这个节点。
- 持久顺序节点
  基本特性和持久节点一致，额外增加顺序上，每个父节点都会为他的**第一子节点维护一份顺序**。创建子节点时候可以设置这个标记，会自动为节点名上一个数据后缀。
- 临时节点
  临时节点的生命周期和客户端绑定在一起，客户端会话失效，这个节点就会被自动清理掉，**客户端失效非TCP连接断开**，**不能基于临时节点创建子节点，临时节点只能是叶子节点**。
- 临时顺序节点
  临时节点基础上添加顺序的特性
###### 状态信息
除了存储数据内容外，还存储了数据节点本身的一些信息，状态信息。是数据节点的Stat对象的格式化输出

<img src="asserts/image-20200228170645405.png" alt="image-20200228170645405" style="zoom:50%;" />

##### 版本，保证分布式原子性操作
  Zookeeper的版本表示的是对数据节点的数据内容、子节点列表、节点ACL信息的修改次数。版本是用来做锁的作用，考虑CAS操作，使用的是乐观锁进行处理，乐观锁在冲突不大时候使用。
###### version
  当前数据节点数据内容的版本号。创建时候为0，对节点的**数据内容进行更新操作**，就会+1，强调的是变更次数。
###### cversion
  当前数据节点子节点的版本号。
###### aversion
  当前数据系欸但ACL变更版本号
##### Watcher-数据变更通知
客户端线程、客户端WatchManager、ZooKeeper服务器三部分。
###### 特性
1. 一次性
    一个Watcher被触发，ZooKeeper都会将其从相应的存储中移除。因此开发人员在使用人要反复的注册，这样有效的减轻服务器的压力
2. 客户端串行执行
    客户端Watcher回调的过程是一个串行同步的过程，这为我们保证了顺序，同时需要开发人员注意不要因为一个Watcher的处理逻辑影响这弓客户端的Watcher回调。
3. 轻量
.  在通知事件时候，不会说明事件的具体内容,需要客户端主动重新取获取数据。在注册时候，只是打上一些标识，在网络开销和服务端内存开销上市非常廉价的。
##### ACL-保障数据安全

#### Leader选举
ZooKeeper的集群规模至少时两台机子。
1. 每个Server都会发出一个投票{myid，ZXID}表示
2. 接收来自各个服务器的投票
3. 处理投票，先比ZXID，在比myid
4. 统计投票，判断是否有过半机器（大于或等于n/2+1）接收到相同投票信息
5. 改变服务器状态
##### 选举的算法分析
  3.4版本后剩余FastLeaderElection选举算法。
术语介绍：

 1. SID：服务器ID。每台机器不能重复，和myid的值一致。
 2. ZXID：事务ID，用来标识一次服务器状态的变更，
 3. Vote：投票，当机器发现无法检测到Leader机器时候，就会尝试投票
 4. Quorum：过半机器数 quorum=n/2+1
 算法分析：
 服务器出现以下情况时候就会进入选举
	 	 服务器初始化启动
	      服务器运行期间无法和Leader保持连接
 当一台机器进入Leader选举时候，集群可能处于以下两种状态
	      已有一个Leader
	      集群不存在Leader
当集群已经有Leader时候，其他机器启动比较晚，视图选举Leader时候，会被告知**当前服务器Leader**信息。仅需建立连接、并进行状态同步。
无Leader时候或者Leader挂掉无法连上Leader时候，服务器会处于LOOKING状态，他会向集群所有其他机器发送消息，即为投票。投票中包含**两个**基本信息，**所推举的服务器SID和ZXID**。下面来说明整个流程。

 ​	 第一次投票；所有服务器投票自己把投票发送出去。

 ​	 变更投票。服务器接收其他机器的投票。进行一次比对操作
 - 规则1：如果vote_zxid大于self_zxid就认可当前收到的投票，并再次将投票发送出去。
 - 规则2：如果vote_zxid小于self_zxid，不变更**是否再次发出已前投票呢**
 - 规则3：如果zxid相同 比较myid

​         确认Leader。经过第二次投票后，集群中的每个机器再次收到其他机器的投票，此时超过半数的机器就成为Leader。

##### 选举的细节
服务器状态：
   - LOOKING:寻找LEADER，当前服务器认为集群中没有Leader，因此进去Leader 选举流程。
   - FOLLOWING：
   - LEADING：
   - OBSERVING：和FOLLOWER类似，但是不选举，提供扩容功能，但是也算在总的集群数量当中。

投票的数据结构：

<img src="asserts/image-20200302223132532.png" alt="image-20200302223132532" style="zoom:50%;" />

- electionEpoch：逻辑时钟，用来判断多个投票是否在同一轮选举周期中，该值在服务端是一个自增序列，每次进入新一轮的投票后，都会对该值+1
- peerEpoch：被选举的Leader的epoch
- state：服务器的状态

消息队列：

  	在QuorumCnxmanager这个类内部维护了一系列的队列，用于保存接收到的和待发送的消息，以及消息的发送器。**除了接收队列外**，其他都有一个共同的特点按SID分组形成队列集合，如集群中除自身外还有4台机器，为4台服务器分别创建一个发送队列。

- recvQueue：消息接收队列，用于存放那些从其他服务器接收到的消息
- queueSendMap：消息发送队列，用于保存待发送的消息，是一个Map按照SID进行分组，分别为集群中每台机器分配一个单独的队列，从而保证各台机器发送消息互不影响。
- senderWorderMap：发送器集合，每个SendWorker消息发送器，都对应一台远程ZOOKeeper服务器，负责消息的发送，同样SenderWorderMap也是按照SID进行分组
- lastMessageSent：最近发送过的消息，这个稽核中为每个SID保留最近发送的一个消息。

建立连接：

​	QuorumCnxmanager在启动时候会创建一个ServerSocket来监听Leader选举的通信端口。开启端口监听后，就能接收到其他服务器创建连接请求。为了避免两台机器重复创建TCP连接，ZooKeeper设计一个建立规则：**只允许SID大的服务器主动和其他服务器建立连接，否则就断开连接**。一旦连接建立，就会**根据远程服务器SID来创建相应的消息发送器SendWorker和消息接收器RecvWorker**并启动他们。

消息接收与发送：

​	 消息的接收过程是消息接收器RecvWorker来负责为每个远程服务器分配一个单独的RecvWorder，不断的读取TCP消息，并保存到recvQueue队列中。

​     消息的发送每个远程服务器上SendWorder只需要不断的从对应的消息发送队列中取出消息发送极客，统一把消息放入lastMessageSent来作为最近发送消息。一旦ZooKeeper
...
### 选举问题
1. 选举的整个流程 以及好处
2. 选举中不同时间的投票时根据什么来判断，怎么判断这次投票结束，发起下一个投票
3. 集群中的节点时如何知道选出Leader的
4. 第一次投票不变更的服务器是不动还是重新发出票
5. 投票有一个ID是怎么获取的和怎么运转的
   逻辑时钟，用来判断多个投票是否在同一轮选举周期中，该值在服务端是一个自增序列，每次进入新一轮的投票后，都会对该值+1。

6. electionEpoch有何作用？
### 运行中问题
1. Leader挂了时怎么被知道的
2. 数据怎么保证一致性
3. QuorumCnxManager干嘛用的
   Leader选举中使用，每台服务器启动都会启动一个，负责各台服务器之间的底层Leader选举过程中的网络通信。
1. Zookeeper扩容时候如何操作。
 修改配置，怎么停机，怎么重启
5. 如何做锁具体代码内容
6. 消息的接收与发送，RecvWorder为每个远程服务器分配一个单独的RecvWorder？